...
# Ollama base URL used for the /api/tags health-check (derived from endpoint)
...
# VIKTOR script root — resolve relative to app.py's own directory so it works
# correctly under both `viktor start` (local) and the cloud preview sandbox.
...
# System prompt injected on every chat turn
_SYSTEM_PROMPT = (
...
def _ollama_status() -> dict:
    """
    Probe Ollama at GET <base>/api/tags.
    Returns a dict matching the /api/chat/status contract.
    """
...
def _chat(message: str) -> dict:
    """
    Send *message* directly to Ollama's OpenAI-compatible completions endpoint.
    Returns the /api/chat response dict.  Never raises — errors become ok:false.
    """
    payload = {
        "model":      HB_AI_MODEL,
        "stream":     False,
    ...
    try:
        ...
        reply = (
            data.get("choices", [{}])[0]
                .get("message", {})
                .get("content", "")
            or data.get("response", "")   # fallback for native Ollama shape
            or json.dumps(data)
        )
...
def _ensure_test_script() -> bool:
    """
    Auto-create viktor/scripts/test.py on first launch if it does not exist.
    Returns True if the script is ready, False if the filesystem is read-only
    (e.g. cloud preview sandbox) — callers should surface a friendly message.
    """
    if os.path.exists(TEST_SCRIPT):
        logger.info(f"✅ Test script found: {TEST_SCRIPT}")
        return True
    try:
...
def _viktor_test() -> dict:
    """
    Run viktor/scripts/test.py --hello homebase via subprocess.
    Returns the /api/viktor/test response dict.  Never raises.
    """
    _ensure_test_script()
    t0 = time.time()

    # Guard: if the script still doesn't exist (e.g. cloud preview read-only fs),
    # return a clear diagnostic instead of crashing.
    if not os.path.exists(TEST_SCRIPT):
        logger.warning("⚠️ test.py not found and could not be created — running in cloud preview?")
        return {
            "ok": False,
            "error": (
    ...
    try:
        ...
        logger.info(f"🚀 stdout: {result.stdout.strip()[:200]}")
        if result.stderr:
            logger.warning(f"⚠️ stderr: {result.stderr.strip()[:200]}")

        # Try to parse stdout as JSON for a richer reply field
...
class Controller(vkt.Controller):
    ...
    The App Builder cloud preview cannot reach local Ollama — use it for
    UI iteration only.
    """
    ...
    # ------------------------------------------------------------------
    # SetParamsButton handler — "Run VIKTOR Test"
    # ------------------------------------------------------------------
    def run_viktor_test(self, params, **kwargs):
        """
        Execute viktor/scripts/test.py via subprocess and write the JSON
        result back into the chat_message field so the WebView renders it
        in the VIKTOR card.
        """
        result = _viktor_test()
        logger.info(f"🚀 VIKTOR test complete: ok={result.get('ok')}")
        # Serialise the full result dict into the text area so the view
        # can detect it and render the structured card.
        return vkt.SetParamsResult(
            {"chat_message": json.dumps(result, indent=2)}
        )

    # ------------------------------------------------------------------
    # Main WebView — the cockpit UI
    # ------------------------------------------------------------------
    @vkt.WebView("HomeBase Dashboard")
    def homebase_view(self, params, **kwargs):
        """
        Render the HomeBase cockpit:
          • Ollama status pill  (green = reachable, red = not)
          • Chat response card  (sends current message to Ollama directly)
          • VIKTOR test card    (shown when the field holds a viktor.test result)
        """
        # ── 1. Ollama status ──────────────────────────────
        ...
        # Only attempt chat when the field contains plain text (not a JSON result blob)
        is_json_blob = False
        parsed_json = None
        try:
        ...
        if raw_message and not is_json_blob:
            if reachable:
                ...
                if chat_result["ok"]:
                    ...
                    chat_response_html = f"""
                    <div class="card">
                        <h3>💬 Chat Response
                            <span class="meta">{HB_AI_MODEL} · {elapsed}s</span>
                        </h3>
                        <p class="response-text">{reply_text}</p>
                    </div>"""
                else:
                ...
                chat_response_html = (
                    ...
                    f'Make sure Ollama is running (<code>ollama serve</code>) and the model '
        ...
        if is_json_blob and parsed_json is not None:
            ...
            viktor_result_html = f"""
            <div class="card success">
                <h3>🚀 VIKTOR Test Result</h3>
                <table>
                    <tr><th>ok</th>        <td>{ok_icon} {parsed_json.get("ok")}</td></tr>
                    <tr><th>tool</th>      <td>{tool_name}</td></tr>
                    <tr><th>message</th>   <td>{msg_val}</td></tr>
                    <tr><th>exit_code</th> <td>{exit_code}</td></tr>
                    <tr><th>elapsed</th>   <td>{elapsed_val}s</td></tr>
                </table>
                <pre>{json.dumps(parsed_json, indent=2)}</pre>
            </div>"""
        ...
        if chat_response_html or viktor_result_html:
            ...
            idle_card = (
                ...
                f'<p class="hint" style="margin-top:12px">Running locally via <code>viktor start</code>? '
...
<body>
    ...
    <p class="subtitle">Local AI Cockpit · <code>viktor start</code> · {_OLLAMA_BASE}</p>
    ...
    <div class="pills">
        <div class="pill">
            ...
            <span class="pill-detail">{status_detail}</span>
        </div>
    </div>
